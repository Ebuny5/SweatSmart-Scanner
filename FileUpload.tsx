...
const ImageCapture: React.FC<ImageCaptureProps> = ({ onImageCapture }) => {
  ...
  useEffect(() => {
    ...
    // The dependency array is intentionally structured this way to only run on mount/unmount
  ...
  const handleCapture = () => {
    if (videoRef.current && canvasRef.current) {
      ...
      if (context) {
        ...
        onImageCapture(dataUrl, 'image/jpeg');
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        if (base64Image) {
          onImageCapture(base64Image, file.type);
          if (stream) {
            stream.getTracks().forEach(track => track.stop());
            setStream(null);
          }
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 text-center border border-gray-200">
      <h2 className="text-2xl font-bold text-gray-800 mb-2">Analyze an Image</h2>
